...
* **Assets ainda em placeholder**: cercas (`fence`, `fence_yellow`), botão de chão, marcas de piso, deck/telhado
  do playground, viga, degraus, parede/janela do corredor, telhado da quadra e os cenários de fundo
  (`school_corridor`, `sports_court`) continuam como caixas toon. Todo o resto (personagens, mobília, portões,
  árvores, arquibancada, colecionáveis, etc.) usa modelos gerados no QtMeshEditor a partir das imagens.
