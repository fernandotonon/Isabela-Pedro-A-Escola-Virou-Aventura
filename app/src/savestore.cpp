...
// clang-format off
EM_JS(char *, escola_ls_get, (const char *key), {
    try {
...
EM_JS(int, escola_ls_set, (const char *key, const char *value), {
    try { window.localStorage.setItem(UTF8ToString(key), UTF8ToString(value)); return 1; }
    catch (e) { return 0; }
});
EM_JS(void, escola_ls_remove, (const char *key), {
    try { window.localStorage.removeItem(UTF8ToString(key)); } catch (e) {}
});
EM_JS(void, escola_ls_clear_prefix, (const char *prefix), {
    try {
...
SaveStore::SaveStore(QObject *parent)
    : QObject(parent), m_prefix(QStringLiteral("escola_aventura/"))
{
}
...
QString SaveStore::get(const QString &key, const QString &defaultValue) const
{
...
#ifdef __EMSCRIPTEN__
    char *raw = escola_ls_get(k.toUtf8().constData());
    if (!raw)
        return defaultValue;
    ...
    return v;
#else
    QSettings s(QStringLiteral("EscolaHorizonte"), QStringLiteral("EscolaAventura"));
...
bool SaveStore::set(const QString &key, const QString &value)
{
    const QString k = m_prefix + key;
#ifdef __EMSCRIPTEN__
    return escola_ls_set(k.toUtf8().constData(), value.toUtf8().constData()) == 1;
#else
    QSettings s(QStringLiteral("EscolaHorizonte"), QStringLiteral("EscolaAventura"));
    s.setValue(k, value);
    s.sync();
...
void SaveStore::remove(const QString &key)
{
    const QString k = m_prefix + key;
#ifdef __EMSCRIPTEN__
    escola_ls_remove(k.toUtf8().constData());
#else
    QSettings s(QStringLiteral("EscolaHorizonte"), QStringLiteral("EscolaAventura"));
    s.remove(k);
#endif
...
{
#ifdef __EMSCRIPTEN__
    escola_ls_clear_prefix(m_prefix.toUtf8().constData());
#else
    QSettings s(QStringLiteral("EscolaHorizonte"), QStringLiteral("EscolaAventura"));
