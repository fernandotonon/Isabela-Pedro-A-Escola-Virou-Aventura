...
EM_JS(void, escola_audio_init, (), {
    if (Module.escolaAudio) return;
    ...
    Module.escolaAudio = A;
});
EM_JS(void, escola_audio_load, (const char *name, const unsigned char *data, int len), {
    const A = Module.escolaAudio; const ctx = A.context(); if (!ctx) return;
...
EM_JS(void, escola_audio_play, (const char *name, double volume), {
    const A = Module.escolaAudio; const ctx = A.context(); if (!ctx) return;
...
EM_JS(void, escola_audio_music, (const char *name, double volume, int loop), {
    const A = Module.escolaAudio; const ctx = A.context(); if (!ctx) return;
...
EM_JS(void, escola_audio_music_stop, (int pause), {
    const A = Module.escolaAudio; if (!A || !A.ctx) return;
    if (A.music) {
...
EM_JS(void, escola_audio_music_resume, (), {
    const A = Module.escolaAudio; if (!A || !A.ctx || A.music || !A.musicName) return;
...
EM_JS(void, escola_audio_music_volume, (double volume), {
    const A = Module.escolaAudio; if (!A) return;
...
EM_JS(void, escola_audio_tick, (), {
    const A = Module.escolaAudio; if (!A || !A.musicPending || !A.musicName) return;
    if (A.buffers[A.musicName]) { A.musicPending = false; A.musicOffset = 0; escola_audio_music_resume(); }
});
#endif
...
bool WebAudio::load(const QString &name, const QUrl &source)
{
    ...
    escola_audio_load(name.toUtf8().constData(), reinterpret_cast<const unsigned char *>(bytes.constData()), bytes.size());
    return true;
#else
...
#ifdef __EMSCRIPTEN__
    escola_audio_tick();
    escola_audio_play(name.toUtf8().constData(), volume);
#else
...
{
#ifdef __EMSCRIPTEN__
    escola_audio_music(name.toUtf8().constData(), volume, loop ? 1 : 0);
#else
...
{
#ifdef __EMSCRIPTEN__
    escola_audio_music_volume(volume);
#else
    Q_UNUSED(volume);
#endif
}
