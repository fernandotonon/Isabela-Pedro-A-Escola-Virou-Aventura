...
const proc = spawn(chrome, [
  ...
  "--no-first-run", "--user-data-dir=" + (process.env.PROFILE ?? "/tmp/escola-browser-check"), "--enable-unsafe-swiftshader",
...
try {
  ...
  const bootLine = await send("Runtime.evaluate", { expression: "JSON.stringify({ swControlled: !!(navigator.serviceWorker && navigator.serviceWorker.controller), loadingHidden: !document.getElementById('escola-loading') })", returnByValue: true }).catch(() => null);
