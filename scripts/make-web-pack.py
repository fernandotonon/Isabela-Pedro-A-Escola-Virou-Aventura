...
"""Write the web preload lists for a deploy directory:
  ...
  escola-pack.bin / escola-pack.json   all small files (< 64 KB, not images/meshes) in ONE download;
                                       index.html writes them into Qt's in-memory FS before main()
  escola-assets.json                   the big files (textures, meshes, sprite sheets) for Qt's own
                                       preloader (parallel downloads)
  escola-manifest.json                 sizes for the loading-screen progress bar
...
open(os.path.join(d, "escola-pack.bin"), "wb").write(blob)
json.dump(pack, open(os.path.join(d, "escola-pack.json"), "w"))
json.dump([{"source": e["source"], "destination": e["destination"]} for e in big], open(os.path.join(d, "escola-assets.json"), "w"))
app_sizes = {f: os.path.getsize(os.path.join(d, f)) for f in os.listdir(d) if f.endswith((".wasm", ".js")) and not f.startswith("escola-sw")}
total = sum(e["size"] for e in big) + len(blob) + sum(app_sizes.values())
json.dump({"expectedBytes": total, "bigFiles": len(big), "packedFiles": len(pack), "packBytes": len(blob), "app": app_sizes}, open(os.path.join(d, "escola-manifest.json"), "w"))
